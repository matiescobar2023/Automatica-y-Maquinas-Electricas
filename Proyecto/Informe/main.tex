...
Cuyas soluciones fueron las indicadas en la \cref{polos del sistema LTI}.
En donde el polo en el origen está asociado al integrador puro a la salida
del diagrama de bloques del LTI equivalente aumentado, correspondiente con
...
Además, se obtuvieron las frecuencias naturales (\cref{ecuacion de las frecuencias naturales del LTI}) y relaciones de amortiguamiento crítico (\cref{ecuacion de las relaciones de amortiguamiento del LTI}), al comparar la ecuación característica asociada a los dos polos complejos conjugados (\cref{polinomio caracteristico del sistema LTI polos complejos conjugados}), con  la ecuación estándar de un sistema de segundo orden sub-amortiguado (\cref{ecuación estandar de un sistema de segundo orden}).
...
\begin{align}
	\omega_{n} &= \sqrt{\dfrac{ R_{s} b_{eq} + \dfrac{3}{2} {P_{p}}^2 { \lambda^r_m}^2}{J_{eq} L_{q}}} \label{ecuacion de las frecuencias naturales del LTI}\\
	\zeta &= \dfrac{ J_{eq} R_{s} + L_{q} b_{eq}}{2 \sqrt{ J_{eq} L_{q}\left( R_{s} b_{eq} + \dfrac{3}{2} {P_{p}}^2 { \lambda^r_m}^2\right) }} \label{ecuacion de las relaciones de amortiguamiento del LTI}
\end{align}

Se pudo ver que solo la perturbación introduce un cero (\cref{cero del sistema LTI}).
...
migración de propiedades del sistema debida a la variación de los parámetros de la carga
$J_l$ y $b_l$ (\cite{c1}) %aca vendría a la cita en la guía donde dice lo de la variación de parametros.

Lo que se puede determinar es, por un lado, que el polo en el origen introducido por el integrador
puro a la salida del sistema no cambia ante la variación de los parámetros de la carga. Por otro lado,
los polos complejos conjugados del sistema se encuentran suficientemente alejados del origen como para
poder observar cómo varían. Por esto, se representa el mapa de los polos complejos conjugados en
una gráfica separada en la ,\cref{mapa de polos complejos}.
