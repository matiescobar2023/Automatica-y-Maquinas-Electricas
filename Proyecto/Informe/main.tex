...
\begin{itemize}
    ...
    \begin{flalign}
        \resizebox{0.88125\linewidth}{!}{
            $T_m(t) = \frac{3}{2} P_p [\lambda'_{mr} i_{qsr}(t) + (L_d - L_q) i_{dsr}(t) i_{qsr}(t)$]}&
        \label{torque electromagnético}
    \end{flalign}
    \item Balance de tensiones eléctricas equivalentes de estator (referido a coordenadas $qd0^{r}$):
    \begin{flalign}
        \resizebox{0.88125\linewidth}{!}{
            $v_{qsr}(t) = R_s(t) i_{qsr}(t) + L_q \frac{di_{qsr}(t)}{dt} + [\lambda'_{mr} + L_d i_{dsr}(t)] \omega_r(t)$} &
        \label{balance de tensiones q}
    \end{flalign}
    \begin{flalign}
        \resizebox{0.88125\linewidth}{!}{
            $v_{dsr}(t) = R_s(t) i_{dsr}(t) + L_d \frac{di_{dsr}(t)}{dt} - L_q i_{qsr}(t) \omega_r(t)$} &
