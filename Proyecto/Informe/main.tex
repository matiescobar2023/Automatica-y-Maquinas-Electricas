...
\begin{itemize}
    ...
    \begin{equation}
        \mathbf{u} = \begin{bmatrix} \mathbf{u_c}(t) \\ \mathbf{u_d}(t)\end{bmatrix}
...
En en modelo de pequeñas desviaciones locales respecto de los puntos de equilibrio se considera
\[
\begin{cases}
...
Por un lado, el \textbf{espacio de operación global NL cuasi-estacionario} se obtiene considerando:
\begin{equation}
\begin{cases}
...
Por otro lado, el \textbf{Modelo dinámico LPV} tiene la expresión:
...
Solamente para este ejercicio, se considera un vector de entradas de perturbación modificado
al considerar $T_l(t)$ desarrollada según la \cref{torque de carga}.
Lo que se hace es reemplazar $T_l(t)$ por $T_d(t)$ en el vector de la \cref{vector de entradas de perturbacion} con lo
cual cambia la definición de $\mathbf{u}(t)$ según la \cref{vector de entradas}.

Las matrices $A(t)$ y $B(t)$ de la \cref{modelo de pequeñas desviaciones} vienen dadas por la \cref{matriz_AB}.
Al considerar el vector de entradas modificado como se indicó, la componente de $T_l(t)$
dependiente de la coordenada articular del brazo es considerada en las matrices $A(t)$ y $B(t)$ en aquellos términos
que contienen a la derivada respecto de $\theta_m(t)$ (recordar relación dada por la \cref{relacion de velocidad en caja}).
