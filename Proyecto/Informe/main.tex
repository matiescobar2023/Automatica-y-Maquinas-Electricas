...
Para lograr esta especificación es necesario realizar una ley de control minima que se determina a partir de la ecuacion de estado para $i^{r}_{ds}$, partiendo de la hipotesis de que $i^{r}_{ds}(0)=0$:
\begin{equation}
	\begin{aligned}
		\dot{i}^r_{ds}(t) = -\frac{R_s(t) i^r_{ds}(t)}{L_d} + \frac{L_q P_p \omega_m(t)i^r_{qs}(t)}{L_d}  + \frac{v^r_{ds}(t)}{L_d}\\
